...
## 管理者の代理操作(impersonation・2026-05-07 シンジさんの方針で正式化)

### 何のための機能か

- **使い方に不慣れな staff の代わりに、admin(シンジさん)が画面操作を代行**できるようにする
- 例: 山田さんが「問題を 10 個まとめて入れたいけど操作が分からない」 → シンジさんが代わりに山田さんとして操作 → 結果は山田さんの個人問題として保存される

### 重要な性質

- **完全ログイン切替ではない**: シンジさんは admin としてログインしたまま、Cookie 上で「今 山田さんとして見ている」状態に入る。**同時に admin / staff 両方の身分**を保持
- **画面に常に「代理操作中」バナー**を表示(自分が誰として動いているか忘れない)
- **代理中の作業は staff の所有として保存**(`owner_id = staff_user_id`)。staff 本人があとでマイページで編集・削除できる
- **すべての代理操作セッションは `impersonations` テーブルにログ**(誰が・いつ・誰として・何分間)
- **タイムアウトなし**(手動「終了」ボタン or ブラウザを閉じるまで継続)

### 実装の要点

- DB の RLS は `auth.uid()`(=admin の id)で動くため、代理中の書き込みは **Route Handler が `service_role` + 明示的な `owner_id = staff_user_id` でバイパスして実行**する
- フロントは Cookie の改ざんで偽装できない(Route Handler が DB の `impersonations` レコードと突合するため)

### NEVER(impersonation 関連)

- ❌ 代理操作中であることを UI で隠す(必ずバナーを出す)
- ❌ 代理操作のログを残さない(全セッションを `impersonations` テーブルに記録)
- ❌ Cookie の値だけで代理状態を信じる(必ず Route Handler 側で `impersonations` の `ended_at IS NULL` を確認)
- ❌ staff から代理操作を起動できる(admin だけ)
- ❌ admin 専用機能を代理中に呼ぶ UI を見せる(代理中は admin メニューを隠す)
